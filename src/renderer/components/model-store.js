...
function renderPresets() {
    presetsTab.innerHTML = '<div class="model-grid"></div>';
    const grid = presetsTab.querySelector('.model-grid');

    presetModels.forEach(model => {
        const isInstalled = installedModels.some(m => m.id === `${model.id}.gguf`);
      ...
      <div class="model-card-header">
        ...
        <span class="model-badge ${model.difficulty}">${model.difficulty.toUpperCase()}</span>
      </div>
      <div class="model-description">${model.description}</div>
      <div class="model-meta">
        <span class="model-meta-item">📦 ${formatBytes(model.size)}</span>
        <span class="model-meta-item">💾 メモリ ${formatBytes(model.memoryRequired)}</span>
        <span class="model-meta-item">⚖️ ${model.quantization}</span>
      </div>
      <div class="model-actions">
        ${isInstalled ?
                '<span class="btn btn-success" disabled>✓ インストール済み</span>' :
                isDownloading ?
...
function handleDownloadProgress(data) {
    ...
    const progressText = document.querySelector(`[data-progress-text="${modelId}"]`);
    const progressPercent = document.querySelector(`[data-progress-percent="${modelId}"]`);
